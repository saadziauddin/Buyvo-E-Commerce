...
// Function to upload images to specific folders in Supabase Storage
...
router.post('/api/products/addProduct', (req, res) => {
  upload.array('images', 10)(req, res, async (uploadError) => {
    if (uploadError) {
      if (uploadError instanceof multer.MulterError) {
        if (uploadError.code === 'LIMIT_FILE_SIZE') {
          return res.status(400).json({ error: 'File size limit exceeds 2MB!' });
        }
        if (uploadError.code === 'LIMIT_UNEXPECTED_FILE') {
          return res.status(400).json({ error: 'You can only upload up to 10 images at a time!' });
        }
    ...
    try {
      ...
      if (req.files && req.files.length > 0) {
        for (const file of req.files) {
          const uploadedImage = await uploadToSupabase(file, folder);
      ...
      const sanitizedProduct = {
        name: name?.trim(),
        color: color ? [...new Set(color.map(c => c.trim()))] : null,
        size: size ? [...new Set(size.map(s => s.trim()))] : null,
        tags: tags ? tags?.trim() : null,
        stock: parseInt(stock, 10),
        category: [...new Set(category.map(c => c.trim()))],
        ...
        youtubeVideoLink: youtubeVideoLink ? youtubeVideoLink?.trim() : null,
